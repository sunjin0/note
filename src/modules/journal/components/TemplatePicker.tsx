...
import { X, Star, Clock, FileText, Plus, ChevronRight, Heart, Trash2, Edit3 } from 'lucide-react';
...
export default function TemplatePicker({
  ...
  const handleSaveCustom = (templateData: {
    title: string;
    content: string;
      ...
      <div className="fixed inset-0 z-[55] flex items-center justify-center p-4">
        ...
        <div className="relative z-10 w-full max-w-2xl max-h-[80vh] overflow-hidden bg-card rounded-2xl shadow-elevated border border-border animate-scale-in">
                ...
                {templates.map((template) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      >
                        <Star
                          className={cn('h-4 w-4', favorites.has(template.id) && 'fill-current')}
                        />
                      </button>
          ...
          <div className="sticky bottom-0 flex items-center justify-between p-4 border-t border-border bg-card">
            <span className="text-xs text-muted-foreground">{t('templates.clickToApply')}</span>
