...
export default function JournalList({ entries, onNewEntry, onEditEntry, onDataChange }: JournalListProps) {
  ...
  }, [filters.search, filters.startDate, filters.endDate, filters.selectedMoods.join(','), filters.selectedFactors.join(',')]);
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-sm font-semibold text-foreground">{formatDate(entry.date, hasFilters)}</span>
                {!hasFilters && <span className="text-xs text-muted-foreground">{weekDay(entry.date)}</span>}
        ...
        <div className="space-y-4">
          ...
          {!hasFilters && groupedEntries.recent.length > 0 && (
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {groupedEntries.recent.map(renderEntryCard)}
              </div>
            </div>
          )}
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-semibold text-muted-foreground px-1 flex items-center gap-2">
                ...
                <span className="text-xs font-normal">
                  ({getPageRangeText().start}-{getPageRangeText().end} / {filteredEntries.length} {t('journal.entries')})
                </span>
              </h3>
              <div className="space-y-2">
                {groupedEntries.recent.map(renderEntryCard)}
              ...
              {totalPages > 1 && (
                <div className="flex flex-col items-center gap-3 pt-4 pb-2">
                  {/* Page Size Selector */}
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-muted-foreground">{t('journal.pageSize') || '每页'}</span>
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <div className="flex items-center gap-1 px-2">
                      <span className="text-sm text-muted-foreground">{t('journal.page') || '页'}</span>
                      <input
                        type="number"
                        min={1}
                        max={totalPages}
                    ...
                    <div key={`${year}-${month}`} className="space-y-2">
                      ...
                      >
                        <div className={cn(
                          ...
                          expandedGroups.has(`${year}-${month}`) ? "bg-primary/10" : "bg-muted/60"
                        )}>
                        ...
                        <span className="text-xs text-muted-foreground bg-muted/60 px-2 py-0.5 rounded-full">
                          {Array.from(weeks.values()).reduce((acc, entries) => acc + entries.length, 0)} {t('journal.entries')}
                        </span>
                      </button>
                            ...
                            <div key={`${year}-${month}-${week}`} className="space-y-2">
                              ...
                              >
                                <div className={cn(
                                  ...
                                  expandedGroups.has(`${year}-${month}-${week}`) ? "bg-primary/10" : "bg-muted/40"
                                )}>
